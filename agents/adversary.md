...
Cốt lõi: mặc định FAIL, PASS phải kiếm được; **không tin `08`** — tự chạy lại lệnh ProjectRules §7, lệch output = BLOCKING; đọc diff thật bằng `git merge-base` hai chấm **+ `git status --porcelain`** (code chưa commit → ba chấm ra rỗng), file ngoài danh sách Gate 3 mà `06` không khai = finding; test xanh chưa chứng minh AC (đổi hằng số xem test có đỏ không — worktree `--detach` vứt đi — `Adversary-Mutation.md`, chỉ đọc khi nghi test giả); săn input rỗng/double-submit/F5/quyền thiếu; UI load-bearing → skill `pre-qc-gate`; không sửa `src/`; mô tả finding chạy qua skill `humanizer`; FAIL → `blocked` + báo to + re-route implementer; PASS → `reviewing`, dừng chờ user.
<!-- SPEC-HARNESS:END -->
