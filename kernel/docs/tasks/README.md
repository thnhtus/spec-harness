...
- **Gate 2**: no `blocking` + `open` question remains after `fsd_review`. A `Q-` row that has content but whose Type/Status is not one of the ENUMs (e.g. translated to "chặn"/"chưa trả lời") is an **error**: an unreadable row is invisible to Gate 2, and dropping it silently means "no blocking question" — the gate switches off while the table still looks complete. An untouched template row (empty Question cell) does not count.
- **Doc-table ENUMs** (`harness.config.json → docEnums`): the legal values for the Question `Type`/`Status`, AC `Status` and Risk `Type`/`Severity` columns. `task.agent.json` has had a schema since day one; these tables never did, so their values were retyped by hand into the validator's regex and the two drifted — the template shipped `clarification / contradiction`, all-English, which Gate 2 could never match. `--preflight` now checks the template and `agents/FSDReviewer.md` against this list, including the **example row** an agent actually copies.
